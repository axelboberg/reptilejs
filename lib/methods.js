  ...
  Model.findMany = function (query = {}, opts) {
    ...
    if (opts.populate) {
      for ([name, rel] of Object.entries(this.relations)) {
        ...
        relationship data
        */
        if (!this.database.models[rel.model]) {
          throw new ReferenceError(`Model '${rel.model}' does not exist or has not been registred`)
        }
          ...
          .map(row => {
            for ({ name, transform, type } of opts._relations) {
              ...
              */
              ...
              /*
              Apply a transform if specified
              in the relation specification
              */
              if (transform) {
