  ...
  Model.findMany = function (query = {}, opts) {
    ...
    if (opts.populate) {
      for ([name, rel] of Object.entries(this.relations)) {
         ...
         * relationship data
         */
        if (!Model.database.models[rel.model]) throw new ReferenceError(`Model '${rel.model}' does not exist or has not been registred`)
        rel.model = Model.database.models[rel.model]
    ...
    return this.execute(_query.getQuery(), _query.getValues(), opts)
      .then(res => {
        ...
        return res
           ...
           */
          .map(row => {
            return new this(null, row, { validate: [], removeExcessProperties: false })
          })
      })
  }
